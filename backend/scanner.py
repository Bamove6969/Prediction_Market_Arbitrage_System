...
# Manifold removed - play-money only
        ...
        if _all_opportunities:
            ...
            try:
                import os
                import requests
                notebook_path = '/app/Cloud_GPU_Matcher_v4.ipynb'
                ...
                if os.path.exists(notebook_path):
                    ...
                    # Inject ngrok public URL so v4 can fetch /api/raw-markets
                    # and POST /api/cloud-results back through the tunnel
                    try:
                        import httpx
                        ngrok_admin = os.environ.get("NGROK_ADMIN_URL", "http://localhost:4040")
                        tunnels_resp = httpx.get(f"{ngrok_admin}/api/tunnels", timeout=5.0)
                        tunnels = tunnels_resp.json().get("tunnels", [])
                        if tunnels:
                            ngrok_url = tunnels[0].get("public_url", "").rstrip("/")
                            import re
                            # Replace the hardcoded Tailscale URL with ngrok
                            notebook_content = re.sub(
                                r'LOCAL_BACKEND_URL\s*=\s*"[^"]*"',
                                f'LOCAL_BACKEND_URL = "{ngrok_url}/api"',
                                notebook_content
                            )
                            logger.info(f"Notebook LOCAL_BACKEND_URL → {ngrok_url}/api")
                    except Exception as e:
                        logger.warning(f"Could not inject ngrok URL: {e}")
                    ...
                    if not github_token:
                        ...
                        gist_data = {
                            'description': f'Arbitrage Scanner v4 - {datetime.utcnow().isoformat()}',
                            'public': True,
                            'files': {
                                'Cloud_GPU_Matcher_v4.ipynb': {
                        ...
                        if gist_resp.status_code == 201:
                            ...
                            colab_url = f'https://colab.research.google.com/gist/Bamove6969/{gist_id}/Cloud_GPU_Matcher_v4.ipynb'
                            ...
                            try:
                                ...
                                if exec_resp.status_code == 200:
                                    ...
                                    scan_state["message"] = f"Colab v4 auto-executing (queue: {exec_data.get('queue_position', '?')})"
                                ...
                                logger.warning(f"Could not reach executor: {e}")
